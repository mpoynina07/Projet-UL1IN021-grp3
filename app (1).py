...
from BDD import init_BDD, get_etat_mailbox, ajouter_courrier_et_maj_mailbox, set_mailbox_empty, get_connection 
...
load_initial_status() # 🚨 APPEL CRUCIAL : synchronisation de la variable globale au lancement du script
...
def update_mailbox_state(etat: bool):
    ...
    try:
        if not etat:
            ...
            set_mailbox_empty(MAILBOX_ID) 
        # Note : Si etat=True, c'est géré directement par ajouter_courrier_et_maj_mailbox()
...
def mesure_distance():
    ...
    # Ce code est sensible au timing et au capteur utilisé.
    try:
    ...
    if distance < seuil and not courrier_present:
        with lock:
            if not courrier_present: # Double vérification sous verrou
                ...
                try:
                    courrier_id = ajouter_courrier_et_maj_mailbox(MAILBOX_ID)
...
# Routes de pages (pour éviter les erreurs d'URL type dashboard.html)
@app.get("/")
async def serve_login(): return FileResponse("static/login.html")
@app.get("/dashboard")
async def serve_dashboard(): return FileResponse("static/dashboard.html")
# ... (Les autres routes /history, /settings, etc. restent ici)
...
async def empty_mailbox_api():
    """API pour marquer la boîte comme vidée (utilisée si le bouton physique n'est pas utilisé ou en plus)."""
    global courrier_present
